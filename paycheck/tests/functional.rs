...
static PROGRAM_ID: Lazy<Pubkey> = Lazy::new(|| Pubkey::from_str("54oykPNNXxpXihbuU5H6j3MZmqCxaAdHALDvVYfzwnW4").unwrap());
#[tokio::test]
...
async fn test_create_paycheck() {
    ...
    let mut program_test =
        ProgramTest::new("paycheck", *PROGRAM_ID, processor!(paycheck::processor::process_instruction));
    ...
    let create_paycheck_ix = paycheck::instructions::create_paycheck::create_paycheck_ix(
        payer.pubkey(),
        args.clone()
    ).unwrap();
    ...
    match result {
        Ok(_) => {
            ...
            let paycheck_account = banks_client.get_account(paycheck_pda).await.expect("Failed to fetch Paycheck account");
            
            if let Some(account) = paycheck_account {
                let paycheck = Paycheck::try_from_slice(&account.data).expect("Failed to deserialize Paycheck account");
