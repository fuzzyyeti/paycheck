...
use crate::utils::get_paycheck_address;

#[tokio::test]
async fn test_create_paycheck() {
    let (mut banks_client, payer, recent_blockhash, owner) =
        setup_program(|p, owner|
        {
            let (paycheck_address, bump) = get_paycheck_address(owner, &WHIRLPOOL_ADDRESS);
    ...
    let (paycheck_address, _) = get_paycheck_address(
        &owner.pubkey(),
        &WHIRLPOOL_ADDRESS);
    ...
    banks_client.process_transaction(close_paycheck_tx).await.unwrap();
    assert!(banks_client.get_account(paycheck_address).await.unwrap().is_none());
